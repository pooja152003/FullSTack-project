...
def chat():
    ...
    if state['stage'] == 'greeting':
        ...
        try:
            ...
            state['stage'] = 'result'
            return jsonify({'reply': str(e.value)})
...
def generate_report():
    ...
    diagnosis = data.get('diagnosis', 'Not available')
    ...
    try:
        ...
        # Call the report generator with full path
        create_pdf_report(name, symptoms, days, diagnosis, description, precautions, filepath)

        return jsonify({'success': True, 'report_url': f"/static/reports/{filename}"})
